...
count. Four design documents for a five-module project is the smell. Each
context earns its place by carrying real vocabulary of its own. A domain may
organize several contexts, but ownership follows the local domain vocabulary,
...
| `accent`     | optional context ownership tint from the declared accent vocabulary. The aggregate registers it once, so `ctx`, `term`, and glossary ownership chips resolve the same palette without repeating a tint at each citation |
...
The `#contract` constructor is listed with the component-card guidance below
because it shares the component data and rendering anatomy while representing a
named agreement as one standalone full-width card.
...
| `#entity` + `#attribute` `#relates`    | `title:`, `kind:`, `owner:`, `lifecycle:`, `domain:` | one entry of a domain-local entity census (§5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `#pending-ledger` + `#pending-entry`   | `title:`, `kind:`, `since:` on each entry            | the pending ledger (§4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| `#cards`                               | `items:`                                             | **the workhorse grid**: `items:` is an array of `(title: …, body: …, tint?: …)`. `cols:`, `tint:`, `size:` optional. The grid `tint:` is the fallback ownership accent; an item's optional `tint:` is validated against the same accent vocabulary and overrides that fallback for that item                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| `#diagram`                             | `altitude:`, string-labelled `nodes:`                | the default STRUCTURE carrier — what is made of what. Graphviz places nodes, routes edges, and places labels; the framework assigns each node kind its architecture role and flat two-dimensional shape. A node with `sub:` renders its primary and secondary labels on separate lines. `bounded-context` is a direct context-map node role; it is distinct from a group boundary even when both use a box mark. Optional `viewpoint:` names the question independently from altitude. Optional `groups:` records declare nested semantic boundaries as `(id:, label:, kind:, parent?:, tint?:)`; every grouped node names a `group:`. A node may add `kind:` for its structural shape and `tint:` for known ownership. A dictionary edge `(from:, to:, relation:, label:)` declares a directional `dependency`, `call`, `pubsub`, or `dataflow`; legacy tuple edges remain valid. Group, node, and relation ids are validated within one diagram only. The generated legend renders the actual used group, node, and relation marks from the same carrier mappings as the graph. A pubsub edge states only its labelled direction, not durability, order, or delivery guarantees. Optional `flow: "left-to-right" \| "top-to-bottom"` guides ranks. Use it unless an authored grid carries meaning or a human needs precise visual control. The one drawing kind that takes an altitude |
| `#diagram-native`                      | `altitude:`, `(id:, pos:, label:)` nodes             | the positioned STRUCTURE carrier. It preserves the authored grid and accepts Typst content labels. A node may carry optional `tint:` from the declared accent vocabulary; otherwise it uses the diagram `accent:`. `edges:` is `(from, to, label)`. Use it only where fine control is necessary. The one drawing kind that takes an altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| `#state-machine`                       | `states:`, `transitions:`                            | a MACHINE — states and what moves between them. `states:` is a flat array of strings; `transitions:` is `(from, to, event)`; `initial:` names the entry state and `accepting:` the terminal ones. **Declares no positions** — the carrier solves the layout. No altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `#sequence`                            | `participants:`, `steps:`                            | an EXCHANGE in time order. `participants:` is `(id:, label:, shape:)`; `steps:` is built from `seq-msg`, `seq-note`, `seq-alt`, `seq-loop`, `seq-opt`. Carries activation, branching, repetition. No altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| `#chart`                               | `kind:` (`bar`\|`column`\|`line`\|`pie`), `points:`  | a MAGNITUDE. `points:` is an array of `(label, value)` pairs; a non-numeric value is a hard failure. `bar` reads categories down the side, `column` across the bottom. No altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| `#formula`                             | positional Typst math body                           | a displayed mathematical relation with optional `id:`, `title:`, `caption:`, `accent:`, `numbered:`, and ordered `(symbol, meaning)` pairs in `notation:`. Inline math remains ordinary Typst math in prose; use the block when an expression needs its own hierarchy, notation key, or reference target. No altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `#pseudocode`                          | `title:`, `steps:`                                   | an implementation-neutral procedure. Optional `id:`, `inputs:`, `outputs:`, `caption:`, and `accent:`. Build control flow with `pseudo-step`, `pseudo-return`, `pseudo-if`, `pseudo-for`, and `pseudo-repeat`; the renderer owns keywords and indentation. Use it for internal procedure, never instead of an externally observable behavior rule. No altitude                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| `#md-table`                            | positional column count and cells                    | a full-width prose table. The positive column count and cell array must form complete rows; the first row is the semantic header. Bounded content-length proportions allocate the full measure without letting one prose column starve another, and only internal horizontal rules are drawn.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `#contract`                            | `name:`, `mission:`                                  | a named integration contract between independently owned units; uses the component card anatomy, renders as one full-width card, and carries a visible `contract` role mark. `accent:` supplies the fallback tint; `tint:` may carry the known domain accent; `lens:`, `body:`, and `answers:` are optional                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| `#coverage`                            | positional rows                                      | the breadth axis in the document: `(part, status, why)` per row                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| `#components` + `#component`           | `name:`, `mission:` on each                          | an inventory of independently owned units: one card per component, each optionally carrying the five completeness contracts. `tint:` is optional on each component and overrides the grid accent for that card; `lens:` remains semantic lens furniture and keeps its lens colour                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `#answers`                             | —                                                    | up to five optional completeness contracts for one already named subject: responsibility, interface, interactions, invariants, and failure. This is not literal question-and-answer prose; each present contract stacks its label above its value with a clear gap and stays left-aligned                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
`#components` inventories independently owned implementation units. `#contract`
describes a named agreement between those units and uses the same five
completeness fields inside one full-width card. Use `#answers` for the five
completeness contracts of another already named subject, such as a policy or
recovery behavior; do not invent a component merely to give a subject a card.

`#contract` accepts `name:`, `mission:`, `accent:`, `tint:`, `lens:`, `body:`,
and `answers:`. `accent:` supplies the fallback tint, while `tint:` carries a
known domain accent; the visible `contract` role mark distinguishes the
agreement from a component that implements it.

### Migrating an integration contract

- Upgrade the renderer pin and project the library before changing a host
  layer. The existing `#component`, `#components`, and `#answers` calls remain
  valid, so this migration is selective rather than a bulk rename.
- Replace an `#answers` card only when its subject is a named agreement between
  independently owned units. Give that agreement a `mission:` and move its
  five fields into `answers-data(...)`, preserving the existing domain accent.
- Keep policy and recovery subjects in `#answers`. Render the host layer and
  run its normal gate after the change, then inspect the full-width contract
  card and its `contract` role mark.

Before:

```typst
#answers(
  title: "Sales boundary",
  accent: accent,
  responsibility: [Supply one confirmed Order and receive Shipment facts.],
  interface: [Request Shipment and receive progress callbacks.],
)
```
...
```typst
#contract(
  name: "Sales–Fulfillment integration",
  accent: accent,
  mission: "Exchange confirmed Orders and accepted Shipment facts.",
...
For a solved `#diagram`, the generated legend is one compact borderless Typst
table/grid under one LEGEND heading, with regular equal-width cells. Each used
group, node, or relation kind places one fixed-size, text-free carrier mark
above its name and short description; all labels stay outside the symbol. A
group cell shows only the carrier's actual boundary stroke, with no fake inner
box. Relation arrows occupy cells in the same grid, and their line and arrow
treatments come from the same Graphviz relation mapping as the diagram. The
legend includes only used kinds and relations, wraps deterministically at a
maximum of five cells per row, and fits normal A4. It does not use Graphviz
whole-legend packing or hardcoded per-kind shapes/layouts; empty and legacy
diagrams remain compatible.
...
node-and-edge graph — that belongs in `#diagram`, where the library
...
The context's optional `accent` binding is the canonical ownership declaration.
The aggregate registers it before chapter bodies and glossary entries render.
An omitted or unknown owner remains neutral; a citation may still provide an
explicit `accent:` override for compatibility.
...
- **Inline math stays inline** when it is part of a sentence. Use `#formula`
  only when the expression is a named unit a reader should scan, cite, or
  decode through a notation key. A formula states a relation, so it has no
  diagram altitude. An `id: "budget"` creates the reference target
  `@formula-budget`.
- **Pseudocode stays implementation-neutral.** Use `#pseudocode` when a
  consequential procedure or decision process must be understood but choosing
  a programming language would falsely make an implementation decision. Keep
  externally observable outcomes in `#behavior`; pseudocode explains the
  internal logical path. An `id: "selection"` creates the reference target
  `@pseudocode-selection`.
- **Code preserves syntax.** Use `#code-block("language", source)` when the
  exact syntax of an existing interface, configuration, or implementation is
  itself evidence. Use pseudocode when only intent and control flow should
  survive.
...
components, `L4` internals — which `#diagram` and `#diagram-native` require on every drawing,
...
### `#entity` — the census of a local domain

A **domain context** is the bounded context owning its local entities, their
relationships, and the state and logic flowing over them. It is homed at its
declared context path, with its own `design.typ` and `CONTEXT.typ`. The context
map identifies the relationship between domains; no context has privileged
ownership of the whole model.
...
**Entity arguments, all required except `id`, `tint`, and `lens`:**

| Argument       | Values                                      | Means                                                         |
| -------------- | ------------------------------------------- | ------------------------------------------------------------- |
| `id:`          | string                                      | optional; stable identity used by linked state fields         |
...
- **State-machine models live OUTSIDE entity blocks**, in the census's own
  lifecycle subsection. Keep one model per distinct lifecycle or state field.
  Multiple independent lifecycle or state fields may therefore have multiple
  models; state combinations that are constrained across fields require
  explicit cross-axis constraints. Include vacant states or revoked-blessing
  transitions only when the domain requires them.
...
| `derived`     | computed from other facts and not independently authored; synchronization follows the declared derive-at-read or derive-at-write policy |
...
#### Linked state type, attribute, and machine

- A legacy entity, attribute, or state machine without state links remains
  valid. An entity gains an `id:` only when another block must identify it.
- A linked attribute supplies `id:`, `state-type:`, and `state-machine:`
  together. Supplying any one requires all three, and the attribute must be
  inside an entity that supplies `id:`.
- A linked state machine supplies `id:`, `subject:`, `state-field:`, and
  `state-type:` together. `subject:` names the entity, and `state-field:` names
  the attribute within that entity.
...
#state-type(
  ...
  variants: (
    (id: "draft", description: [Not submitted.]),
    (id: "confirmed", description: [Accepted.]),
  ),
)
  ...
  id: "booking",
  title: "Booking",
  description: [A reservation for a requested time window.],
  kind: "aggregate",
  owner: "scheduling",
  lifecycle: "stateful",
  domain: "scheduling",
...
#state-machine(
  ...
  transitions: (("draft", "confirmed", "confirm"),),
  initial: "draft",
  accepting: ("confirmed",),
)
```

- A state type declares only its value set. Each variant is either a string id
  or a record carrying `id:` and an optional `description:`.
- A state machine alone declares transitions, `initial:`, and `accepting:`.
  The state type's variant ids and the machine's states must be equal as sets.
- The aggregate requires unique entity ids, state type ids, and state machine
  ids. An attribute id is unique within its entity and may repeat in another
  entity.
- The aggregate resolves every link and requires the attribute, machine, and
  state type to agree. A linked attribute renders clickable references to the
  state type and state machine.
