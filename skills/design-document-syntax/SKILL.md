...
| `#diagram`                             | `altitude:`, string-labelled `nodes:`                | the default STRUCTURE carrier — what is made of what. Graphviz places nodes, routes edges, and places labels; the framework assigns each node kind its architecture role and shape. Optional `viewpoint:` names the question independently from altitude. Optional `groups:` records declare nested semantic boundaries as `(id:, label:, kind:, parent?:, tint?:)`; every grouped node names one `group:`. A node may add `kind:` for its structural shape and `tint:` for known ownership. A dictionary edge `(from:, to:, relation:, label:)` declares a directional `dependency`, `call`, `pubsub`, or `dataflow`; legacy tuple edges remain valid. Group, node, and relation ids are validated within one diagram only. The generated legend states every used role. A pubsub edge states only its labelled direction, not durability, order, or delivery guarantees. Optional `flow: "left-to-right" \| "top-to-bottom"` guides ranks. Use it unless an authored grid carries meaning or a human needs precise visual control. The one drawing kind that takes an altitude |
