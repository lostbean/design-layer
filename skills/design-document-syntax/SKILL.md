...
| `#diagram-native`                      | `altitude:`, `nodes:`                                | a STRUCTURE — what is made of what. `layout: "manual"` is the default and requires `(id:, pos:, label:)` nodes. `layout: "solved"` uses the Graphviz carrier and requires string `(id:, label:)` values; optional `flow: "left-to-right" \| "top-to-bottom"` guides its ranks. `edges:` is `(from, to, label)`. Every endpoint must name a declared node. The one drawing kind that takes an altitude |
