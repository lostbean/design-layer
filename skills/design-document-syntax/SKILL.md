...
| `#diagram-native`                      | `altitude:`, `nodes:`                                | a STRUCTURE — what is made of what. `nodes:` is an array of `(id:, pos:, label:)`; `pos:` is a grid coordinate and is required. `edges:` is `(from, to, label)`. Every endpoint must name a declared node. The one drawing kind that takes an altitude                   |
