...
`#components` inventories owned units. `#answers` explains the five optional
completeness contracts for a named subject, whether or not that subject is a
component. Keep a protocol, policy, recovery behavior, or other non-component
subject in `#answers`; do not invent a component merely to give it a card.
